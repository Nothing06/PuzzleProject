...
public abstract class Node {
    ...
    static void initialize() {
        dimension = Puzzle_Config.getDimension();
        numOfTiles = Puzzle_Config.getNumOfTiles();
        numOfTilesMinusOne = Node.numOfTiles - 1;
        goalState = Puzzle_Config.getGoalState();
        goalStatePositions = Puzzle_Config.getGoalStatePositions();
        heuristic = Puzzle_Config.getHeuristic();
    }

    public static int h(final long boardConfig) {
        if (heuristic == Puzzle_Config.HEURISTIC_PD) {
            if (numOfTiles == 16) {
                ...
                return Puzzle_Config.costTable_15_puzzle_0[index0] +
                       Puzzle_Config.costTable_15_puzzle_1[index1] +
                       Puzzle_Config.costTable_15_puzzle_2[index2];

            } else {
                return Puzzle_Config.patternDatabase_8_puzzle.get(boardConfig);
            }
        }
        ...
        if (heuristic == Puzzle_Config.HEURISTIC_LC) {
